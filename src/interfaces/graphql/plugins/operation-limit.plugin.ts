...
const identityFor = (rule: Rule, context: GraphQLContext, email?: string): string => {
  const ip = context.ip;
  const user = context.caller?.id;
...
export const operationLimitPlugin: Plugin<GraphQLContext> = {
